...
Texture2D<uint> gMaterialIndexBuffer : register( t2 );
Texture2D<float1> gDepthBuffer : register( t3 );
SamplerState gSampler : register( s0 );
SamplerComparisonState gSamplerComparison : register( s1 );

ConstantBuffer<Camera> gCamera : register( b0 );
ConstantBuffer<Light> gLight : register( b1 );
StructuredBuffer<Material> gMaterials : register( t4 );

Texture2D<float> gShadowDepth : register( t5 );
